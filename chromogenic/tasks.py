...
def instance_export_task(instance_export):
    ...
    (orig_managerCls, orig_creds, export_managerCls,
     export_creds) = instance_export.prepare_manager()
...
def migrate_instance_task(origCls, orig_creds, migrateCls, migrate_creds,
                          **imaging_args):
    logger.info("migrate_instance_task task started at %s." % datetime.now())
    new_image_id = migrate_instance(origCls, orig_creds, migrateCls,
                                    migrate_creds, **imaging_args)
