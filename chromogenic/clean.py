...
from chromogenic.common import (remove_files, overwrite_files,
                                ...
                                execute_chroot_commands, fsck_image,
                                mount_image)
...
def remove_ldap(mounted_path):
    try:
        ...
        run_command([
            "/usr/sbin/chroot", mounted_path, 'yum', 'remove', '-qy',
            'openldap'
        ])
    finally:
...
def mount_and_clean(image_path,
    ...
    import subprocess
    output = subprocess.Popen(['virt-inspector', '-a', image_path],
    ...
    if '<name>cloud-init</name>' not in output:
        raise Exception(
            "cloud-init is not installed on this image so it will fail to deploy on Atmosphere"
        )

    # Get filename and content based off distro
    vs_filename = "{}/virt-sysprep-{}.txt".format(os.path.dirname(image_path),
                                                  distro)
    vs_content = virt_sysprep_files.commands % (getattr(
        virt_sysprep_files, distro), fstrim)
    ...
    proc = subprocess.Popen([
        'virt-sysprep', '-a', image_path, '--operations',
        'defaults,kerberos-data,user-account', '--hostname', distro,
    ...
    if rc != 0:
        logger.error("virt-sysprep exited with code {} and message: {}".format(
            rc, err))
        raise Exception(
            "virt-sysprep failed on image file {} with error: {}".format(
                image_path, err))
