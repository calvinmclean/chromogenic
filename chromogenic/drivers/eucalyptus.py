...
class ImageManager(BaseDriver):
    ...
    def __init__(self, *args, **kwargs):
        ...
        (key, secret, ec2_url,
         s3_url) = self._env_credentials(key, secret, ec2_url, s3_url)
        self.s3_url = s3_url
    ...
    def create_image(self, instance_id, image_name, *args, **kwargs):
        ...
        #Step 1. Download
        download_args = self.download_instance_args(instance_id, image_name,
                                                    **kwargs)
        ...
        ##Step 3. Upload image
        upload_kwargs = self.parse_upload_args(instance_id, image_name,
    ...
    def download_instance_args(self, instance_id, image_name, **kwargs):
        ...
        meta_name = kwargs.get(
            'meta_name',
            self._format_meta_name(image_name,
        ...
        download_dir = os.path.join(download_dir, reservation.owner_id,
                                    instance_id)
        ...
        download_args['download_location'] = kwargs.get(
            'download_location',
        ...
        download_args['remote_img_path'] = kwargs.get(
            'remote_img_path',
            self._format_nc_path(reservation.owner_id, instance_id))
        return download_args

    def download_instance(self, instance_id, download_location, *args,
                          **kwargs):
        """
        ...
        return self.scp_remote_file(remote_img_path, download_location,
                                    **node_scp_info)
    ...
    def download_image_args(self, image_id, **kwargs):
        ...
        # <dir>/<username>/<instance>
        # This helps us keep track of ... everything
        download_location = os.path.join(download_dir, image_id, name)
    ...
    def parse_upload_args(self, instance_id, image_name, image_path, **kwargs):
        ...
        upload_kwargs['bucket_name'] = kwargs.get(
            'bucket_name',
            self._format_meta_name(image_name,
                                   reservation.owner_id,
                                   creator='admin'))
    ...
    def upload_local_image(self, image_name, image_path, kernel, ramdisk,
                           destination_path, parent_emi, bucket_name, public,
                           private_user_list):
        """
        ...
        new_image_id = self._upload_and_register(image_path, bucket_name,
                                                 kernel, ramdisk,
                                                 destination_path,
                                                 ancestor_ami_ids)
    ...
    def upload_full_image(self,
        ...
        ramdisk_id = self._upload_ramdisk(ramdisk_path, bucket_name,
                                          download_dir)

        new_image_path = os.path.join(
            os.path.dirname(image_path), '%s%s' % (self._format_meta_name(
                image_name, uploaded_by), os.path.splitext(image_path)[1]))
        ...
        try:
            ...
            new_image_id = self.upload_local_image(image_name, new_image_path,
    ...
    def _prepare_kvm_export(self, image_path, download_dir):
        ...
        (kernel_dir, ramdisk_dir,
         mount_point) = self._export_dirs(download_dir)
        ...
        local_ramdisk_path = _copy_ramdisk(mount_point, rmdisk_version,
                                           ramdisk_dir)
        local_kernel_path = _copy_kernel(mount_point, rmdisk_version,
                                         kernel_dir)
    ...
    def _boto_s3_conn(self, key, secret, s3_url):
        ...
        s3_conn = S3Connection(aws_access_key_id=key,
    ...
    def _boto_ec2_conn(self,
        ...
        image_conn = connect_ec2(aws_access_key_id=key,
                                 aws_secret_access_key=secret,
                                 is_secure=False,
                                 region=region,
                                 port=parsed_url.port,
    ...
    def _get_latest_ramdisk(self, mount_point):
        ...
        for line in output.split('\n'):
            if 'initrd' in line and 'xen' not in line:
                latest_rmdisk = line
                rmdisk_version = line.replace('.img',
                                              '').replace('initrd-', '')
    ...
    def _format_nc_path(self,
                        owner,
                        instance_id,
                        prefix='/usr/local/eucalyptus',
                        disk='root'):
    ...
    def _upload_kernel(self, image_path, bucket_name, download_dir='/tmp'):
        return self._upload_and_register(image_path,
                                         bucket_name,
                                         kernel='true',
    ...
    def _upload_and_register(self,
        ...
        manifest_loc = self._bundle_image(image_path,
                                          download_dir,
                                          kernel,
                                          ramdisk,
        ...
        logger.info("New image created! ID:%s" % new_image_id)
        return new_image_id
    ...
    def _check_mount_path(self, filepath):
        ...
        if filepath.startswith('/'):
            logger.warn("WARNING: File %s has a LEADING slash. " +
                        ...
                        "the HOST MACHINE and must be changed!" % rm_file)
            filepath = filepath[1:]
        return filepath
    ...
    def _bundle_image(self,
        ...
        try:
            ...
            logger.error(
                "Image process aborted! Environment not properly set." +
        ...
        (tgz_file,
         sha_image_digest) = self.euca.tarzip_image(prefix, image_path,
                                                    destination_path)
        logger.debug('Encrypting zip file')
        (encrypted_file, key, iv,
         bundled_size) = self.euca.encrypt_image(tgz_file)
        os.remove(tgz_file)
        ...
        self.euca.generate_manifest(
            ...
            cert_path, euca_cert_path, private_key_path, target_arch,
            image_size, bundled_size, sha_image_digest, user, kernel, ramdisk,
            mapping, product_codes, ancestor_ami_ids)
        logger.debug('Manifest Generated')
        #Destroyed encrypted file
        os.remove(encrypted_file)
        manifest_loc = os.path.join(destination_path,
                                    '%s.manifest.xml' % prefix)
        return manifest_loc

    def _export_to_s3(self,
                      keyname,
                      the_file,
                      bucketname='eucalyptus_exports',
                      **kwargs):
        s3_key = kwargs.get('s3_key')
        s3_secret = kwargs.get('s3_secret')
        s3_url = kwargs.get('s3_url')
        key = self._upload_file_to_s3(bucketname, keyname, the_file, s3_key,
                                      s3_secret, s3_url)
    ...
    def _download_euca_image(self,
        ...
        if not image_location:
            image_location = os.path.join(
                download_dir, manifest_loc.replace('.manifest.xml', ''))
        ...
        manifest_local_file = self._download_manifest(bucket, part_dir,
                                                      manifest_loc)
    ...
    def _download_parts(self, bucket, download_dir, manifest_name):
        ...
        for idx, part in enumerate(parts):
            ...
            k.key = part
            if idx % 5 == 0:
                logger.debug("Downloading part %d/%d.." %
                             (idx + 1, len(parts)))
            k.get_contents_to_file(part_file)
            part_file.close()
        return part_files

    def _unbundle_manifest(self,
                           source_dir,
                           download_dir,
                           manifest_file_loc,
                           pk_path,
        ...
        (parts, encrypted_key,
         encrypted_iv) = self.euca.parse_manifest(manifest_file_loc)
    ...
    def find_bucket(self, name):
        return [
            b for b in self.list_buckets() if name.lower() in b.name.lower()
        ]
    ...
    def find_instance(self, name):
        return [
            m for m in self.list_instances()
...
def _ensure_bucket(connection, bucket, canned_acl=None):
    ...
    try:
        ...
        if (s3error_string.find("404") >= 0):
            try:
                bucket_instance = _create_bucket(connection, bucket,
                                                 canned_acl)
            except S3CreateError:
                logger.info('Unable to create bucket %s' % (bucket))
                sys.exit()
        elif (s3error_string.find("403") >= 0):
            logger.info("You do not have permission to access bucket: %s" %
                        bucket)
            sys.exit()
        else:
...
def _upload_parts(bucket_instance,
                  directory,
                  parts,
                  part_to_start_from,
                  canned_acl=None):
