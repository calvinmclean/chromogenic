...
def export_instance(src_managerCls, src_manager_creds, exportCls,
                    export_kwargs):
    """
    ...
    return begin_export(download_location, download_manager, exportCls,
                        export_kwargs)
...
def add_eth0_module(mounted_path):
    prepend_line_list = [
        ("alias eth0 e1000", "etc/modprobe.d/virtualbox"),
    ]
...
def remove_sensu(mounted_path):
    try:
        prepare_chroot_env(mounted_path)
        run_command([
            "/usr/sbin/chroot", mounted_path, 'yum', 'remove', '-qy', 'sensu'
        ])
    finally:
        remove_chroot_env(mounted_path)
